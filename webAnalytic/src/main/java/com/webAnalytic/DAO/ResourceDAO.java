...
public class ResourceDAO implements DAO<Resource> {
    ...
    public boolean deleteByListId(List<Long> listParams) throws Exception {
        assert (listParams != null);

        String sqlQuery = "delete from [Resource] where webSite_id=?";
        return jdbcLayer.update(sqlQuery, listParams)==list().size();
    }
